...
function makeQuestions(lang: 'fr' | 'en') {
...
export default function Marathon() {
  ...
  const [questions] = useState(() => makeQuestions(lang))
  ...
  if (isLast) {
    ...
    const msg = lang === 'en'
      ? (score >= 95 ? 'Perfect! You\'re a pro 🏆' : score >= 80 ? 'Excellent! 🔥' : score >= 50 ? 'Not bad at all! 💪' : 'Keep listening, it\'ll come! 🌱')
      : (score >= 95 ? 'Parfait ! Tu es un pro 🏆' : score >= 80 ? 'Excellent ! 🔥' : score >= 50 ? 'Pas mal du tout ! 💪' : 'Continue l\'écoute, ça va venir ! 🌱')
    ...
    return (
      <div className="max-w-[400px] mx-auto">
        <div className="phrase-card p-6 text-center">
          ...
          <p className="relative z-10 text-[12px] text-[var(--text-light)] mt-1">{correct}/{questions.length} {lang === 'en' ? 'correct answers' : 'bonnes réponses'}</p>
          ...
          <p className="relative z-10 text-[11px] text-[var(--text-light)] mt-2">
            {ui('hub.record')} {Math.max(score, record)}%
          </p>
        </div>
  ...
  return (
    <div className="pb-8">
      ...
      <div className="detail-header p-4 text-white text-center mb-4">
        <h1 className="relative z-10 text-[18px] font-[800] m-0">{ui('quiz.marathon')}</h1>
          ...
          {selected && (
            <>
              ...
              <motion.button initial={{ opacity: 0 }} animate={{ opacity: 1 }} onClick={() => { setSelected(null); setCurrent((c) => c + 1) }} className="phrase-badge !text-[13px] !px-5 !py-2 !rounded-lg cursor-pointer mt-3 ml-auto block">
                {current + 1 >= questions.length ? (lang === 'en' ? 'See score' : 'Voir le score') : (lang === 'en' ? 'Next →' : 'Suivant →')}
              </motion.button>
            </>
          )}
        </motion.div>
