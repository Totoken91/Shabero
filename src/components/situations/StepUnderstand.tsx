...
function getMeaning(p: Phrase, lang: 'fr' | 'en') {
  return lang === 'en' && p.en ? p.en : p.fr
}

function makeQuestions(phrases: Phrase[], allPhrases: Phrase[], lang: 'fr' | 'en') {
  return shuffle(phrases).map((p) => {
    const correct = getMeaning(p, lang)
    const distractors = shuffle(allPhrases.filter((d) => getMeaning(d, lang) !== correct)).slice(0, 3).map((d) => getMeaning(d, lang))
...
export default function StepUnderstand() {
  ...
  const [questions] = useState(() => scenario ? makeQuestions(scenario.phrases, allPhrases, lang) : [])
  ...
  if (isLast) {
    ...
    if (stepXP > 0) showXPToast(stepXP, lang === 'en' ? 'Step 2 !' : 'Etape 2 !')
      ...
      <div className="max-w-[400px] mx-auto">
        <div className="phrase-card p-6 text-center">
          ...
          <p className="relative z-10 text-[13px] mt-2" style={{ color: passed ? '#34A853' : '#E53935' }}>
            {passed ? (lang === 'en' ? 'Step passed!' : 'Étape validée !') : (lang === 'en' ? '80% needed to pass. Listen again!' : 'Il faut 80% pour valider. Réécoute les phrases !')}
          </p>
        </div>
  ...
  return (
    <div className="pb-8">
      ...
      <div className="phrase-card p-3 mb-4 text-center">
        ...
        <p className="relative z-10 text-[11px] text-[var(--text-light)] mt-1">{ui('hub.step')} 2 — {ui('hub.stepUnderstand')} • {current + 1}/{questions.length}</p>
      </div>
